...
def entrenar(features, scaler_x, scaler_y, X_train_tensor, y_train_tensor, X_test_tensor, y_test_tensor, device, df,seq_len, producto_id):
    ...
    if predicciones is not None:
        ...
        ultimo_inventory = df.loc[mask, 'Units Sold']
        ...
        for fecha, pred in zip(fechas_futuras, predicciones):
            ...
            print("Sobrante:", sobrante, type(sobrante))

            if sobrante.item() != 0 and sobrante.item() > 0:
        ...
        plt.bar(fechas_futuras.insert(0, fechas_futuras[0] - pd.Timedelta(days=1)), inventario, color='orange', alpha=0.6, label='Units Sold proyectado')
    ...
    return producto_id, fecha_inicio, paso, frecuencia, device, features, scaler_x, scaler_y,seq_len, df